    ...
    def evict_kv_pages(self, reqs: list[message.EvictResourceRequest]):
        for req in reqs:
            for layer in range(self.lm.config.num_layers):
                src_gpu_ptrs = req.src_ptrs
                dst_cpu_ptrs = req.dst_ptrs

                self.kv_cache_at_layer_cpu[layer][dst_cpu_ptrs].copy_(
                    self.kv_cache_at_layer[layer][src_gpu_ptrs],
                    non_blocking=True)
    
    @torch.inference_mode()
    def restore_kv_pages(self, reqs: list[message.RestoreResourceRequest]):
        for req in reqs:
            for layer in range(self.lm.config.num_layers):
                src_cpu_ptrs = req.src_ptrs
                dst_gpu_ptrs = req.dst_ptrs

                self.kv_cache_at_layer[layer][dst_gpu_ptrs].copy_(
                    self.kv_cache_at_layer_cpu[layer][src_cpu_ptrs],
                    non_blocking=True)
